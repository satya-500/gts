...
installgems()
{
    ...
    if [ "${ruby_major_version}" -lt "2" ]; then
        ...
        gem install json -v 2.2.0 ${GEMOPT}
    fi
    sleep 1
    gem install soap4r-ng -v 2.0.3 ${GEMOPT}
    gem install multi_xml -v 0.6.0 ${GEMOPT}
    gem install httparty -v 0.14.0 ${GEMOPT}
    gem install httpclient -v 2.8.3 ${GEMOPT}
    gem install ffi -v 1.11.1 ${GEMOPT}
    wget https://rubygems.org/downloads/posixpsutil-0.1.0-x86_64-linux.gem
    gem install ./posixpsutil-0.1.0-x86_64-linux.gem
    # This is for the unit testing framework.
    gem install simplecov -v 0.17.1 ${GEMOPT}
    gem query --local
}
...
installzookeeper()
{
    ...
    pipwrapper "kazoo==2.6.0"
}

installpycrypto()
{
    pipwrapper "pycrypto==2.6.1"
}
